...
export class NodeBase {
  ...
  static getNodeOfElement(el: Node | undefined) {
    if (!el) return;
    if (!el.nodeType)
      throw new Error('must pass an Element to NodeBase.getNodeOfElement');

    let elTrackingNode = el as ElementTrackingNode;
  ...
  isQuietEmptyDelimiter(dlms: { [id: string]: any } | undefined) {
    ...
    let key = this.parent.ctrlSeq.replace(/^\\(left|right)?/, '');
 ...
 */
 export class Fragment {
    ...
    adopt(parent: MQNode, leftward: NodeRef, rightward: NodeRef) {
      ...
      let ends = { [L]: parent.getEnd(L), [R]: parent.getEnd(R) };
    ...
    disown() {
      ...
      let ends = { [L]: parent.getEnd(L), [R]: parent.getEnd(R) };
      if (leftEnd[L]) {
        let leftLeftEnd = leftEnd[L] as MQNode;
      ...
      if (rightEnd[R]) {
        let rightRightEnd = rightEnd[R] as MQNode;
