suite('Localization', function () {
  ...
  suite('Static Methods', function () {
    test('checks language support', function () {
      assert.equal(MQ.L10N.isLanguageSupported('en'), true);
      assert.equal(MQ.L10N.isLanguageSupported('es'), true);
      assert.equal(MQ.L10N.isLanguageSupported('fr'), false);
    });

    test('resolves language variants', function () {
      assert.equal(MQ.L10N.resolveLanguage('en-US'), 'en');
      assert.equal(MQ.L10N.resolveLanguage('es-MX'), 'es');
      assert.equal(MQ.L10N.resolveLanguage('fr'), 'en'); // fallback
    });
  });
});
